...
public class InsertPlan extends LogicalPlan {
    ...
    final static String errColumnNum = "Exception: insert operation error (columns don't match)!";//列数不匹配
    final static String errColumnType = "Exception: insert operation error (types don't match)!";//类型不匹配
    final static String errValueNum = "Exception:  insert operation error  (number of columns and values don't match)!";//列数与值数不匹配
    final static String errDuplicateValueType = "Exception: insert operation error  (duplicate name of columns)!";//列名重复
    final static String errColumnName = "Exception: insert operation error  (wrong column name)!";//属性名不在列定义中
    final static String errDuplicateKey = "Exception: insert operation error  (insertion causes duplicate key)!";//主键重复
    final static String errStringLength = "Exception: insert operation error  (string length exceeds the limit)!";//字符串过长

    public InsertPlan(String tableName, ArrayList<String> columnNames, ArrayList<ArrayList<String>> values) {
            ...
            for (ArrayList<String> value : values) {
                ...
                for (int j = 0; j < columns.size(); j++) {
                    ...
                    if (match != -1) {
                        ...
                        if (c.isNotNull()) {
                            throw new InsertErrorException("Exception: insert operation error ( column " + c.getColumnName() + " cannot be null )");
                        }
                        else {
...
//                undo();
    ...
    private void matchType(Column column, String value, String primaryKey, ArrayList<Entry> entries) {
        ...
        if (value == null) {
            if (column.isNotNull()) {
                throw new InsertErrorException("Exception:  insert operation error ( " + column.getColumnName() + " cannot be null)");
            }
            else {
                entries.add((new Entry(null)));
